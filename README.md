...
The arm64, armv7, and bundled outputs have been compared byte-for-byte with the Python builder. The original source APK's non-signature entries are all `ZIP_STORED`; later tools may DEFLATE entries such as `AndroidManifest.xml`, and the builder preserves compressed entries that do not need content edits. For metadata and endpoint patch targets, the builder inflates compressed entries with `bytes_inflate`, patches their uncompressed bytes, and writes the result back uncompressed. A newly added ZIP entry uses UTC rather than Python's local-time timestamp; set `SOURCE_DATE_EPOCH` (and `TZ=UTC` for the Python comparison) for deterministic byte-identical output. The Python builder `Server/build_phone_apk.py` was removed once the Legible port was verified against it; it remains recoverable from git history at commit `55a5a6b`, for example `git show 55a5a6b:Server/build_phone_apk.py`. The request-synthesis layer and both listeners are ported as
